...
public class NItems  {
	...
	public void InitLabel(){
	    ...
	    this.label[i]=new JLabel("Slot Nr.:"+(i+1)+"   "+SpielPanel.getDynamicObject(SpielPanel.getPlayerIndex()).getName(i) +"  Anzahl:   "+"  "+SpielPanel.getDynamicObject(SpielPanel.getPlayerIndex()).AnzahlItems(i),pics[i],JLabel.RIGHT);
	...
	public void InitDimension(){
		this.l=new Dimension[SpielPanel.getDynamicObject(SpielPanel.getPlayerIndex()).NumberItems()];
		for(int i=0;i<SpielPanel.getDynamicObject(SpielPanel.getPlayerIndex()).NumberItems();i++){
	...
	public void AddToPan(){
		int y=0;
		int z=0;
		for(int i=0;i<=(SpielPanel.getDynamicObject(SpielPanel.getPlayerIndex()).NumberItems()/3);i++){
		   this.label[i].setBounds(40,80+y,l[i].width,l[i].height);
		   pan.add(label[i]);
		   y+=70;
		}
		for(int i=(SpielPanel.getDynamicObject(SpielPanel.getPlayerIndex()).NumberItems()/3)+1;i<SpielPanel.getDynamicObject(SpielPanel.getPlayerIndex()).NumberItems();i++){
			   this.label[i].setBounds(260,80+z,l[i].width,l[i].height);
			   pan.add(label[i]);
			   z+=70;
			}
		
		this.money= new JTextField("Dein Vermoegen: "+this.SpielPanel.getDynamicObject(SpielPanel.getPlayerIndex()).getMoney()+" $$$");
	...
	public String[] Bild(){
		...
		for(int i=0;i<SpielPanel.getDynamicObject(SpielPanel.getPlayerIndex()).NumberItems();i++){
			if(SpielPanel.getDynamicObject(SpielPanel.getPlayerIndex()).getName(i)=="Leer"){
